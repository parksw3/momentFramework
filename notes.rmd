...
What does $\kappa_3/\kappa_2$ look like?

```{r plot0}
plot(k3/k2, type = "l")
```

How well does our equation approximate $\kappa$?
...
kappa.approx <- totS^(r-2)/(exp(constant) - totS^(r-2))

matplot(cbind(k2, kappa.approx), type = "l")
```

It seems like only the initial trajectory is similar... Also, this is a bad approximation because $CV^2$ goes 0. What happens if we replace $r$ in the equation with $\kappa_3/\kappa_2$ instead of keeping it constant?

```{r plot2}
r2 <- k3/k2
kappa.approx2 <- totS^(r2-2)/(exp(constant) - totS^(r2-2))

matplot(cbind(k2, kappa.approx2), type = "l")

```

At least this doesn't cause $\kappa$ to go to 0... Can we approximate $r$ so that $\lim_{S\to 1} r = \hat{r}$ and $\lim_{S\to 0} r = 2$ using a linear equation? $$r \approx (\hat{r}-2)S + 2$$
...
```{r trajPlots}
matplot(cbind(r.lnorm.het[,N.l+2], r.lnorm.app1[,3], r.lnorm.app2[,3]), type = "l", lwd = 2)
```

Linear approximation coupled with $\kappa$ approximation seems like it does a fairly good job...

Squared CV

```{r CV2}
matplot(cbind(r.lnorm.het[,N.l+3], r.lnorm.app1[,4], r.lnorm.app2[,4]),type = "l", lwd =2)
```

Mean susceptibility

```{r susPlot}
matplot(cbind(r.lnorm.het[,N.l+4], r.lnorm.app1[,5], r.lnorm.app2[,5]),type = "l", lwd=2)
```
